'''
panda/player/_learning_player.py
---
...
from panda.environment.state import BLANK_STATE, State
from panda.memory.continuous_bank import ContinuousBank
from panda.memory.episode import Episode
from panda.memory.episode_bank import EpisodeBank
from panda.memory.transition import Transition
from panda.player._last_prev_states import LastPrevStatesByPerspective
...
class _LearningPlayer( Player ):
	def __init__( self, PROFILE ):
		...
		self._transition_bank = (
			ContinuousBank( self.PROFILE ) if self.PROFILE.CONTINUOUS_MEMORY 
	...
	def process_step_and_learn( 
		...
		# previous state becomes the next state.
			...
			self._last_prev.set_slot( 
				index, state, conv, add_fc, action, reward, legal
			)
	...
	def _allocate_new_perspective( self, perspective ):
		...
		if n_slots < perspective:
			...
			if not self.PROFILE.CONTINUOUS_MEMORY:
				self._active_episodes.extend( 
					[None for _ in range( n_new_slots )] 
				)
		if ( not self.PROFILE.CONTINUOUS_MEMORY 
