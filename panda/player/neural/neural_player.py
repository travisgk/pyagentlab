'''
panda/player/neural/neural_player.py
---
...
from panda.environment.action.action import (
	action_to_combo_num, subjective_action_from_network_output
)
from panda.environment.action.random_action import random_subjective_action
from panda.player._learning_player import _LearningPlayer
from panda.player.neural.neural_network import NeuralNetwork
...
class NeuralPlayer( _LearningPlayer ):
	def __init__( self, PROFILE ):
		...
		self._q_next = ( 
			None if self.PROFILE.REPLACE_TARGET_INTERVAL == 0 
		...
		self._lr_scheduler = lr_scheduler.ReduceLROnPlateau( 
			...
			verbose=False 
		) if self.PROFILE.LR_SCHEDULER_FACTOR > 0.0 else None
	...
	def choose_action( self, state, conv, add_fc, player_num ):
		...
		if np.random.random() > self._epsilon:
			with T.no_grad():
				t_conv = T.tensor( 
					np.expand_dims( conv, axis=0 ), dtype=T.float32 
				).to( self._device ) if uses_conv() else None
				t_add_fc = T.tensor(
					np.expand_dims( add_fc, axis=0 ), dtype=T.float32
				).to( self._device ) if uses_add_fc() else None
				t_outputs = self._q_eval.forward( t_conv, t_add_fc )

			if self.PROFILE.ENFORCE_LEGALITY:
				illegal_mask = state.create_illegal_subjective_action_mask( 
					player_num 
				)
	...
	def calc_avg_loss( self, sample_size=10 ):
		return ( np.mean( list(self._loss_history)[-sample_size:] ) 
				 if len( self._loss_history ) >= 1 else 0.0
		)
	...
	def _train( self, sample_entire_episodes=False, scramble_samples=False ):
		...
		transitions = []
		perspectives = []
		self._transition_bank.sample_transitions( transitions, 
												  perspectives, 
		...
		t_prev_conv = T.tensor(
			np.array( [t.prev_conv_obs for t in transitions] ), dtype=T.float32
		).to( self._device ) if uses_conv() else None
		t_prev_add_fc = T.tensor(
			np.array( [t.prev_add_fc_obs for t in transitions] ),
			dtype=T.float32
		).to( self._device ) if uses_add_fc() else None
		t_next_conv = T.tensor(
			np.array( [t.next_conv_obs for t in transitions] ), dtype=T.float32
		).to( self._device ) if uses_conv() else None
		t_next_add_fc = T.tensor(
			np.array( [t.next_add_fc_obs for t in transitions] ),
			dtype=T.float32
		).to( self._device ) if uses_add_fc() else None
		t_rewards = T.tensor(
		...
		action_combo_nums = [
			action_to_combo_num( t.action ) for t in transitions
		]

		# runs minibatches through the eval neural network to get output.
		minibatch_selector = np.arange( self.PROFILE.MINIBATCH_SIZE, 
										dtype=np.int32 
		)
		t_preds = self._q_eval.forward( 
			t_prev_conv, t_prev_add_fc 
		...
		# then the next states must be approximated.
			...
			t_nexts = ( self._q_next.forward( t_next_conv, t_next_add_fc ) 
						if self._q_next else t_evals
			)
			...
			if self.PROFILE.FORCE_ILLEGALS_IN_NEXTS:
				for i, t in enumerate( transitions ):
					if ( not next_is_dones[i] and 
						 t.prev_state.action_is_legal( t.action )
					):
			...
			t_max_actions = T.argmax( 
				t_evals[:, CONST.CONTINUOUS_ACTION_DIM:], dim=1 
			) + CONST.CONTINUOUS_ACTION_DIM

			# creates the targets tensor for loss.
			t_targets =  ( t_rewards + 
						   self.PROFILE.GAMMA * 
						   t_nexts[minibatch_selector, t_max_actions] 
			)
		...
		if ( self.PROFILE.L1_REG_STRENGTH > 0.0 and
			 self.PROFILE.L2_REG_STRENGTH > 0.0
		):
			l1_regularization = T.tensor( 0., requires_grad=True ).to( 
				self._device 
			)
			l2_regularization = T.tensor( 0., requires_grad=True ).to( 
				self._device 
			)
		...
		if self.PROFILE.GRADIENTS_MAX_NORM > 0.0:
			nn.utils.clip_grad_norm_( self._q_eval.parameters(),
									  max_norm=self.PROFILE.GRADIENTS_MAX_NORM
			)
