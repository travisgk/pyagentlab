'''
panda/player/neural/neural_network.py
---
...
class NeuralNetwork( nn.Module ):
	...
	def _create_fc_layers( self ):
		...
		N_OUTPUTS = (
			CONST.CONTINUOUS_ACTION_DIM + np.prod( CONST.DISCRETE_ACTION_DIMS )
		)
		...
		if len( self._PROFILE.FC_LAYER_SPECS ) == 0:
			self._fc_layers.append( 
				nn.Linear( N_INPUTS, N_OUTPUTS, bias=True ) 
			)
	...
	def _calc_n_conv_outputs( self ):
		if len( self._conv_layers ) == 0:
			...
			n_conv_outputs = ( np.prod( CONST.CONV_INPUT_DIMS ) 
			...
			t_conv_obs = T.tensor( 
				np.expand_dims( State.BLANK_CONV_OBS, axis=0 )
			)
			with T.no_grad():
	...
	def _create_fc_dropout_layers( self ):
		self._fc_dropouts = nn.ModuleList(
			[nn.Dropout( p=spec.DROPOUT_RATE ) 
			 for spec in self._PROFILE.FC_LAYER_SPECS
			]
		)
