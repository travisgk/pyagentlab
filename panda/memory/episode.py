...
class Episode:
	...
	def finalize( self, PROFILE, player_outcome, player_rank, worst_rank ):
		...
		legal_indices = [ i for i in range( len( self.transitions ))
						  if self._legals[i]
		]
		...
		if len( legal_indices ) > 0:
			...
			if player_outcome == OUTCOME.WIN:
				...
				self.transitions[last].reward = ( PROFILE.LOSS_VALUE *
					( 1 if not player_rank or not worst_rank
					  else PROFILE.LOSS_VALUE_RANK_FACTOR ** 
					  ( worst_rank - player_rank )
				))
