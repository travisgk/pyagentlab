...
class CONST:
	CHECKPOINT_DIRECTORY = os.path.join( 'panda', 'models' )
	ENV_NAME = 'env'
	...
	def finalize():
		CONST.N_CONV_CHANNELS = ( CONST.CONV_INPUT_DIMS[0] 
								  if len( CONST.CONV_INPUT_DIMS ) >= 1 else 0
		)
		CONST.CONV_WIDTH = ( CONST.CONV_INPUT_DIMS[1] 
							 if len( CONST.CONV_INPUT_DIMS ) >= 2 else 0
		)
		CONST.CONV_HEIGHT = ( CONST.CONV_INPUT_DIMS[2] 
							  if len( CONST.CONV_INPUT_DIMS ) >= 3 else 0
		)
		CONST.FLATTENED_DISCRETE_ACTION_DIM = np.prod( 
			CONST.DISCRETE_ACTION_DIMS 
		)
