...
def _apply_perspective( obj_number, perspective ):
	if obj_number == 0:
		return 0
	return ( obj_number - perspective ) % CONST.N_PLAYERS + 1
...
class State:
	...
	def setup():
		State.BLANK_CONV_OBS = np.full( CONST.CONV_INPUT_DIMS, 
										State.ONE_HOT_FALSE, 
										dtype=State.MEM_DTYPE 
		) if uses_conv() else None
		State.BLANK_ADD_FC_OBS = np.full( CONST.ADD_FC_INPUT_DIM, 
										  State.ONE_HOT_FALSE, 
										  dtype=State.MEM_DTYPE 
	...
	def take_action( 
		self, action, player_num=None, local_n_steps=None 
	):
		reward = 0.0
		player_done = False
		outcome = None
		...
		if not self.action_is_legal( action, player_num ):
			...
			if CONST.ILLEGAL_ENDS_PLAYER or CONST.ILLEGAL_ENDS_ENTIRE_ENV:
				outcome = ( OUTCOME.LOSS if CONST.ILLEGAL_END_IS_LOSS 
