'''
panda/environment/environment.py
---
...
class Environment:
	...
	def step( self, action ):
		...
		if not self.entirely_done():
			...
			self.state.take_action( 
				action, self._player_num, self.local_n_steps
			)
		...
		return ( copy.copy( returned_state ), 
				 reward, 
	...
	def _determine_final_outcomes( self, player_outcome, legal ):
		...
		if any( [out in uncertain_outs for out in self._player_outcomes] ):
			...
			if not legal and CONST.ILLEGAL_ENDS_ENTIRE_ENV:
				...
				global_outcome = ( OUTCOME.WIN if CONST.WINS_BY_FORFEIT 
								   else OUTCOME.INTERRUPTED 
				)
			elif player_outcome == OUTCOME.WIN and CONST.WIN_ENDS_ENTIRE_ENV:
				completely_done = True
				global_outcome = ( OUTCOME.LOSS if CONST.LOSES_BY_ENDING_WIN
								   else OUTCOME.INTERRUPTED
				)
			elif player_outcome == OUTCOME.LOSS and CONST.LOSS_ENDS_ENTIRE_ENV:
				completely_done = True
				global_outcome = ( OUTCOME.WIN if CONST.WINS_BY_ENDING_LOSS 
								   else OUTCOME.INTERRUPTED
				)
				...
				active_player_indices = [
					i for i, done in enumerate( self._player_dones ) 
					if not done
				]
				if all( [ not self.state.legal_plays_remain( i + 1 ) 
		...
		if completely_done:
			...
			if not any( [out == OUTCOME.DRAW for out in outs] ):
				if not any( [out == OUTCOME.WIN for out in outs] ):
					if CONST.ALL_LOSSES_IS_DRAW:
						...
						if n_losses > 1:
							loss_indices = [ i for i, out in enumerate( outs )
											 if out == OUTCOME.LOSS
							]
							for i in loss_indices:
								self._player_outcomes[i] = OUTCOME.DRAW
				else:
					if ( CONST.ALL_WINS_IS_DRAW and
						 not any( [out == OUTCOME.LOSS for out in outs] )
					):
						n_wins = self._player_outcomes.count( OUTCOME.WIN )
						if n_wins > 1:
							win_indices = [ i for i, out in enumerate( outs )
											if out == OUTCOME.WIN
							]
			...
			unranked_player_indices = [ i for i in range( CONST.N_PLAYERS ) 
										if not self._player_win_ranks[i]
			]
			...
			value_to_rank = { 
				value: rank + 1 
				for rank, value in enumerate( unique_sorted_ranks )
			}
	...
	def append_score_and_outcome_history( self, scores, history_offset=0 ):
		...
		for i, outcome in enumerate( self._player_outcomes ):
			element = ( i + history_offset ) % CONST.N_PLAYERS
			self._episode_outcomes_history[element].append( 
				self._player_outcomes 
			)

		if ( self._n_steps % CONST.ENV_LOG_HISTORY_INTERVAL == 0 and 
			 self._n_steps > 0
		):
			...
			for i in range( CONST.N_PLAYERS ):
				...
				self.avg_episode_scores[element].append(
					( 
						self._n_steps,
						np.mean( self._episode_scores_history[element] )
					)
				)
