"""
pyagentlab/profile/_option_categories.py
---
...
class _OptionCategories:
    ...
    def add_category(self, category_name):
        if category_name not in [
            category.category_name for category in self._categories
        ]:
                ...
                if key_name not in HIDDEN_KEYS:
                    ...
                    try:
                        ...
                        if search_key in ["LR", "EPS_DEC"]:
                            info_list.append(key_name.ljust(TAB) + f" {VALUE:.1e}\n")
                        ...
                        # appends lines of layer specifications.
                            ...
                            STR_LIST = VALUE
                            if len(STR_LIST) == 0:
                                info_list.append(key_name.ljust(TAB) + f" []\n")
                            elif len(STR_LIST) == 1:
                                info_list.append(
                                    key_name.ljust(TAB) + f" [{STR_LIST[0]}]\n"
                                )
                            else:
                                info_list.append(
                                    key_name.ljust(TAB) + f" [{STR_LIST[0]}"
                        ...
                        # appends the name of a function.
                            ...
                            try:
                                ...
                                func_name = VALUE
                            info_list.append(key_name.ljust(TAB) + f" {func_name}\n")

                        # appends line of information as usual.
                        else:
                            info_list.append(key_name.ljust(TAB) + f" {VALUE}\n")
                    except ValueError:
                        pass
    ...
    def rearrange_categories(self, category_names):
        ...
        self._categories = sorted(
            self._categories,
            key=lambda x: (
                category_names.index(x.category_name)
                if x.category_name in category_names
                else len(category_names)
            ),
        )
