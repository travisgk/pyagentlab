"""
pyagentlab/memory/episode_bank.py
---
...
class EpisodeBank(TransitionBank):
    ...
    def sample_transitions(
        ...
        sample_method = (
            self._mass_sample if sample_entire_episodes else self._single_sample
        )

        minibatch_remaining = minibatch_size
        for ep_index in episode_indices:
            sample_method(
                transitions_list, perspectives_list, ep_index, minibatch_remaining
            )
    ...
    def _mass_sample(self, transitions_list, perspectives_list, ep_index, max_memories):
        ...
        perspectives_list.extend(np.full(end, episode.perspective, dtype=np.uint8))
